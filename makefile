# project also make use of:
#  - .github/publish.yml to push with github action
...
import: # output: f"data/{author_id}.parquet"
	python $(IMPORT_DIR)/get_author_dat_oa.py -a $(author_id) -o $(DATA_DIR)

get-citation-works: # output: f"data/incoming_citations/{author_id}/*.parquet"
	python $(IMPORT_DIR)/incoming_citations.py -a $(author_id) -o $(DATA_DIR_CIT_WORKS)

wrangle-edge-data: # output: f"data/{author_id}_topic_net.parquet"
...
wrangle-timeseries: # output: f"data/{author_id}_timeseries.parquet"
